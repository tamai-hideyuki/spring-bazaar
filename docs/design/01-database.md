...
|--------|-----|------|-----------|------------|
| `id` | `UUID` | NO | — | PK |
| `order_number` | `VARCHAR(16)` | NO | — | 表示用注文番号。`UNIQUE`。採番規則: `yyyyMMdd-NNNN`(JST 日次連番) ★v0.5.2 |
...
| 8 | 注文番号(人間可読) | `order_number` 列追加、`yyyyMMdd-NNNN` 採番(要件書 v0.5.2 §8-4-1) | **Phase 1 で実装** |
